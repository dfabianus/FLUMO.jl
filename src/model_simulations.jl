...
function simulate_GalOx_soft_sensor(online, offline)
    mP = offline.cP_theo
    mD = offline.GuHCl
    V = 1.0
    t_copper = offline.t_cop./60
    ...
    u0 = [sys_simp.I => mP±0.01, sys_simp.V => V±0.001]
    ...
    #p2 = plot!(ts, osol(ts, idxs=sys_simp.A).u, label = "A(t)")
    #p2 = plot!(ts, osol(ts, idxs=sys_simp.N+sys_simp.I).u, label = "SOL(t)")
    p2 = plot(ts, osol(ts, idxs=sys_simp.N+sys_simp.A).u, label = "N+A+NC (direct)", color=2)
    p2 = plot!(ts, pmean.(osol_ol(ts, idxs=sys_ol.cA+sys_ol.cNC).u), label = "N+A+NC (open-loop)", linewidth=1.5, linestyle=:dash, color=2)
    vline!([offline["t_cop"]/60], label="copper addition", color=1,linewidth=2)
    # if "c_p" in names(offline)
    #     scatter!(offline.time./60, offline.c_p, label = "P(t) data")
    # else
    #     scatter!(offline.time./60, offline.c_P, label = "P(t) data")
